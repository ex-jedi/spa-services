...
function Instafeed(options) {
  ...
  assert(
    ...
    `target must be a string or DOM node, got ${opts.target} (${typeof opts.target})`
  );
  assert(
    typeof opts.accessToken === 'string' || typeof opts.accessToken === 'function',
    `accessToken must be a string or function, got ${opts.accessToken} (${typeof opts.accessToken})`
  );
  assert(
    typeof opts.accessTokenTimeout === 'number',
    `accessTokenTimeout must be a number, got ${opts.accessTokenTimeout} (${typeof opts.accessTokenTimeout})`
  );
  assert(
    typeof opts.apiTimeout === 'number',
    `apiTimeout must be a number, got ${opts.apiTimeout} (${typeof opts.apiTimeout})`
  );
  ...
  assert(
    ...
    `template must null or string, got ${opts.template} (${typeof opts.template})`
  );
  assert(
    !opts.error || typeof opts.error === 'function',
    `error must be null or function, got ${opts.error} (${typeof opts.error})`
  );
  assert(
    !opts.before || typeof opts.before === 'function',
    `before must be null or function, got ${opts.before} (${typeof opts.before})`
  );
  assert(
    !opts.after || typeof opts.after === 'function',
    `after must be null or function, got ${opts.after} (${typeof opts.after})`
  );
  assert(
    !opts.success || typeof opts.success === 'function',
    `success must be null or function, got ${opts.success} (${typeof opts.success})`
  );
  assert(
    !opts.filter || typeof opts.filter === 'function',
    `filter must be null or function, got ${opts.filter} (${typeof opts.filter})`
  );
  assert(
    !opts.transform || typeof opts.transform === 'function',
    `transform must be null or function, got ${opts.transform} (${typeof opts.transform})`
  );
  assert(
    !opts.sort || typeof opts.sort === 'function',
    `sort must be null or function, got ${opts.sort} (${typeof opts.sort})`
  );
  assert(
    !opts.render || typeof opts.render === 'function',
    `render must be null or function, got ${opts.render} (${typeof opts.render})`
  );
  assert(
    !opts.limit || typeof opts.limit === 'number',
    `limit must be null or number, got ${opts.limit} (${typeof opts.limit})`
  );
  assert(
    !opts.apiLimit || typeof opts.apiLimit === 'number',
    `apiLimit must null or number, got ${opts.apiLimit} (${typeof opts.apiLimit})`
  );
...
Instafeed.prototype._extractTags = function extractTags(str) {
  const exp = /#([^\s]+)/gi;
  const badChars = /[~`!@#$%^&*\(\)\-\+={}\[\]:;"'<>\?,\./|\\\s]+/i; // non-allowed characters
  const tags = [];
  let match = null;
...
Instafeed.prototype._basicRender = function basicRender(data) {
  const exp = new RegExp(
    `${this._options.templateBoundaries[0]}([\\s\\w.]+)${this._options.templateBoundaries[1]}`,
    'gm'
  );
