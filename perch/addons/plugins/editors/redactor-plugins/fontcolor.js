(function ($R) {
  $R.add('plugin', 'fontcolor', {
    ...
    // private
    _buildDropdown() {
      const $dropdown = $R.dom('<div class="redactor-dropdown-cells">');
    ...
    _buildPicker(name) {
      const $box = $R.dom(`<div class="re-dropdown-box-${name}">`);
      const rule = name == 'backcolor' ? 'background-color' : 'color';
      ...
      for (let z = 0; z < len; z++) {
        const color = this.colors[z];

        const $swatch = $R.dom('<span>');
